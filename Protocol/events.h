...
HANDLE GetReadEvent(HWND hwnd, WPARAM wParam) {
	HANDLE ReadEvent;
...
BOOL CreateReadEvent(HWND hwnd, WPARAM wParam) {
	HANDLE readEvent = GetReadEvent(hwnd, wParam);
	if (!SetEvent(readEvent)) {
		//DisplayMessage(hwnd, "Read Event was not set.");
		return false;
	}
	return true;
}